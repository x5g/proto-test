...
user = '1098766468@qq.com'  #Replace it
password = 'Czx970711'  #Replace it
g = Github(user,password)

def get_code(keywords):
    repositories = g.search_code(query = keywords)
    print('get code {}'.format(repositories.totalCount))
    return repositories
...
def get_issueInfo(repo_issues,repo_name,repo_owner):
 ...
 #some issues do not have label and body
        ...
        issue_info_list.append([repo_name,repo_owner,issue_id,issue_title,issue_state, issue_label,issue_html_url,issue_body])
...
if __name__ == '__main__':
    ...
    repositories = get_code(keywords)
...
#         try:
#             if flag == 1:
#                 csv_headers = ['repo_name','repo_owner','issue_id','issue_title','issue_state', 'issue_label','issue_html_url','issue_body']
#                 data.to_csv(fileName, header = csv_headers, index = False, mode = 'a+', encoding = 'utf-8-sig')
#             else:
